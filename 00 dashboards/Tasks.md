...
copy.createDiv({ cls: "adx-date", text: "TASK STUDIO" });
copy.createEl("h1", { text: "Everything in flight." });
copy.createDiv({ cls: "adx-focus", text: "Every open task, grouped and filtered by the note it lives in. Tick one off and the source line flips to done." });
...
const render = () => {
  ...
  if (!items.length) return empty(list, currentFilter === "all" ? "No open tasks." : `No open tasks in ${prettyCat(currentFilter)}.`);
