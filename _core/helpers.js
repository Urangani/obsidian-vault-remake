...
return (function build({ dv, require, app }) {
  ...
  function mountNav(rootEl) {
    ...
    // Dragging is a phone-only behavior — desktop/tablet keep the static rail.
    const isPhone = document.body.classList.contains("is-mobile") || window.innerWidth <= 620;
    // Restore a previously dragged position; overrides the translateY centering.
    const savedPos = isPhone ? store.get("nav-pos", null) : null;
